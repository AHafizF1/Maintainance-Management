...
export function Sidebar({
  ...
  const handleSaveUri = () => {
    const trimmedUri = editableUri.trim();
    ...
    if (typeof window !== "undefined") {
      if (trimmedUri === "") {
        ...
        localStorage.setItem("morphik_uri", trimmedUri);
        // Try to parse the token from the URI
        try {
          // Regex to extract the token from morphik://<token>@host
          const match = trimmedUri.match(/^morphik:\/\/([^@]+)@/);
          if (match && match[1]) {
            const token = match[1];
            localStorage.setItem("jwt_token", token);
            console.log("JWT Token saved to localStorage for API authentication.");
            // Any API client can now read this token and set the
            // 'Authorization: Bearer <token>' header.
          } else {
            // If the format is incorrect or there's no token, remove any existing one.
            localStorage.removeItem("jwt_token");
          }
        } catch (error) {
          console.error("Error parsing token from URI:", error);
          localStorage.removeItem("jwt_token");
        }
      }
    }
    ...
    if (onUriChange) {
      ...
      onUriChange(trimmedUri);
    } else {
      // Fallback for demo purposes if no callback is provided
      console.log("New URI:", trimmedUri || "(empty - using localhost)");
    }
    ...
    >
      ...
      <ScrollArea className="flex-1 px-2 py-4">
        <div className={cn("flex flex-col space-y-1", isCollapsed ? "items-center" : "")}>
          ...
          <Button
            variant={activeSection === "production" ? "secondary" : "ghost"}
            className={cn("w-full justify-start", isCollapsed && "justify-center")}
            onClick={() => onSectionChange("production")}
            title="Production"
          >
            <LayoutGrid className={cn("h-5 w-5", !isCollapsed && "mr-2")} />
